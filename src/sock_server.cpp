...
int	main(int argc, char *argv[]) {
	...
	//ソケット, アドレスの生成
	fd_sock = sock_init(&addr_from, port_num);

	//受信待ち
	int	status_listen = listen(fd_sock, SOMAXCONN);
	if (status_listen < 0) { //ソケット, キューの最大長 //エラー処理
		std::cerr << COLOR_RED << "Error listen: " << std::strerror(errno) << std::endl; //標準エラー出力
	...
	Debug::print("sm41\taddr_to", &addr_to);
	Debug::print("sm41\tlen_addr", len_addr);
	int	fd_conn = accept(fd_sock, (t_addr *)&addr_to, &len_addr); //接続待ちソケット, 接続相手のソケットアドレスポインタ, 接続相手のアドレスサイズ
